...
## Nouveautés de la release 0.4.9

- **Google Calendar** : un appui sur toute la carte déplie désormais le lieu et la description de l’événement. Le bouton « Description » est supprimé et un second appui replie les détails.
- **Météo heure par heure** : jusqu’à 24 créneaux à partir de l’heure courante restent disponibles dans un rail horizontal tactile, y compris après minuit.
- **Affichage adaptatif** : le nombre de cartes visibles dépend toujours de la largeur du widget ; leur largeur et l’alignement du défilement évitent de couper une heure au bord de la carte.
- **Accessibilité** : les événements sont activables au clavier, annoncent leur état ouvert ou fermé et le rail météo est parcourable au clavier lorsqu’il déborde.

La release comprend le serveur, l’interface Web, les contrats et l’APK **0.4.9**, avec **versionCode 16**. Les widgets météo actuelle, prévisions quotidiennes, météo horaire et Calendar portent également la version **0.4.9**. Voir les [notes de publication complètes](releases/0.4.9.md).
...
Pour la présente release, les valeurs attendues sont `0.4.9` et `versionCode = 16`.
...
```powershell
git commit -m "Release 0.4.9: détails Calendar et météo horaire défilable"
...
git status --short
git log -1 --oneline
git tag -a v0.4.9 -m "HomeDash 0.4.9"
...
Dans **Releases > v0.4.9**, vérifiez la présence des quatre fichiers :
...
Le workflow publie les notes de `docs/releases/0.4.9.md` avec la release. Rappelez également que les installations dont l’installeur date d’avant 0.4.5 doivent appliquer la réparation ci-dessous une fois.
...
Après publication complète de **v0.4.9**, connectez-vous au Pi en SSH si cette réparation n’a pas encore été appliquée. Assurez-vous qu’aucune installation n’est en cours (`update-status.json` ne doit pas indiquer `installing`), puis :
...
Si `git status --short` montre des changements, arrêtez-vous avant le `checkout` et préservez-les. L’installeur remplace le script et l’agent natifs, puis installe le serveur 0.4.9 en conservant configuration, association, certificat et données. Sur un Zero, patientez jusqu’au message final ; ne coupez pas l’alimentation.
